...
const MOVING_PERIODE_FPS: f64 = 0.1;
...
impl Game {
    ...
    pub fn update(&mut self, delta_time: f64) {
        ...
        if self.game_over {
            if self.waiting_time > RESTART_TIME_SECOND {
                self.restart();
            }
            return;
        }
        ...
        if self.waiting_time > MOVING_PERIODE_FPS {
    ...
    fn check_if_snake_alive(&self, dir: Option<Direction>) -> bool {
        ...
        if self.snake.overlap_tail(next_head.x, next_head.y) {
            return false;
        }
